...
async function guestAuth(req, res, next) {
  ...
  const list = await db.collection('shoplists').findOne({ token });
  if (!list || list.expiresAt < new Date()) return res.status(401).json({ error: 'Link abgelaufen' });
  const creator = await db.collection('users').findOne({ _id: new ObjectId(list.createdBy) });
  if (!creator) return res.status(401).json({ error: 'Nicht gefunden' });
  req.user = creator;
  req.guestName = req.headers['x-guest-name'] || 'Gast';
  req.guestListId = list._id.toString();
  req.guestListName = list.name;
  req.isGuest = true;
  next();
}
...
app.get('/api/admin/data', auth, async (req, res) => {
  ...
  try {
    ...
    const userList = users.map(u => {
      ...
      return { ...u, _id: u._id.toString(), groupName: g?.name || (u.solo ? 'Solo' : '–'), groupCode: g?.inviteCode || '–', groupId: u.groupId || null, taskCount: tasks.filter(t => t.assignee === u._id.toString()).length, createdAt: u.createdAt };
    });
    res.json({ users: userList, groups: groups.map(g => ({ ...g, _id: g._id.toString(), memberCount: g.memberIds?.length || 0, taskCount: tasks.filter(t => t.groupId === g._id.toString()).length })), totalTasks: tasks.length });
...
app.delete('/api/admin/user/:id', auth, async (req, res) => {
  ...
  try {
    ...
    if (user.groupId) await db.collection('groups').updateOne({ _id: new ObjectId(user.groupId) }, { $pull: { memberIds: uid } });
...
app.post('/api/users/login', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      const r = await db.collection('users').insertOne({ name: username.trim(), color: '#5B5BD6', photo: null, token, groupId: null, solo: false, colorOverrides: {}, notifications: [], theme: '', lastVisit: new Date(), createdAt: new Date() });
...
app.delete('/api/users/me', auth, async (req, res) => {
  try {
    ...
    if (u.groupId) {
      ...
      if (g?.creatorId === u._id.toString() && g.memberIds.length > 1) return res.status(400).json({ error: 'Gruppe erst löschen.' });
...
app.post('/api/users/:id/notify', auth, async (req, res) => {
  try {
    ...
    await db.collection('users').updateOne({ _id: new ObjectId(req.params.id) }, { $push: { notifications: { id: genToken().slice(0,8), type: 'info', text: message, createdAt: new Date() } } });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/groups', auth, async (req, res) => {
  try {
    ...
    const r = await db.collection('groups').insertOne({ name: name.trim(), photo: photo || null, creatorId: req.user._id.toString(), memberIds: [req.user._id.toString()], inviteCode: genToken().slice(0,8).toUpperCase(), createdAt: new Date() });
...
app.post('/api/groups/join', auth, async (req, res) => {
  try {
    ...
    if (!g.memberIds.includes(uid)) await db.collection('groups').updateOne({ _id: g._id }, { $push: { memberIds: uid } });
...
app.delete('/api/groups/mine', auth, async (req, res) => {
  try {
    ...
    await db.collection('listentries').deleteMany({ groupId: g._id.toString() });
    await db.collection('shoplists').deleteMany({ groupId: g._id.toString() });
    await db.collection('checklogs').deleteMany({ groupId: g._id.toString() });
...
app.post('/api/groups/leave', auth, async (req, res) => {
  try {
    ...
    const g = await db.collection('groups').findOne({ _id: new ObjectId(u.groupId) });
    if (g?.creatorId === u._id.toString() && g.memberIds.length > 1) return res.status(400).json({ error: 'Zuerst Gruppe löschen.' });
...
app.post('/api/tasks', auth, async (req, res) => {
  try {
    ...
    if (assignee && assignee !== 'all' && assignee !== req.user._id.toString()) {
      await db.collection('users').updateOne({ _id: new ObjectId(assignee) }, { $push: { notifications: { id: genToken().slice(0,8), type: 'task_assigned', text: `${req.user.name} hat dir eine neue Aufgabe zugewiesen: „${task.title}"`, taskId: r.insertedId.toString(), createdAt: new Date() } } });
    }
...
app.patch('/api/tasks/:id', auth, async (req, res) => {
  try {
    ...
    if (update.done === true && task.creatorId && task.creatorId !== req.user._id.toString()) {
      await db.collection('users').updateOne({ _id: new ObjectId(task.creatorId) }, { $push: { notifications: { id: genToken().slice(0,8), type: 'task_done', text: `${req.user.name} hat „${task.title}" erledigt!`, taskId: req.params.id, createdAt: new Date() } } });
    }
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// ── SHOPPING (Artikelkatalog) ─────────────────────
app.get('/api/shopping', auth, async (req, res) => {
  try {
    const items = await db.collection('shopping').find(shopScope(req.user)).sort({ createdAt: 1 }).toArray();
...
app.post('/api/shopping', auth, async (req, res) => {
  try {
    ...
    const doc = {
      ...shopScope(req.user), title: title.trim(), category: category || 'Sonstiges',
      qty: qty || '', price: price || '', comment: comment || '', repeat: repeat || null,
      photo: photo || null, photos: Array.isArray(photos) ? photos : (photo ? [photo] : []),
...
app.patch('/api/shopping/:id', auth, async (req, res) => {
  try {
    ...
    const allowed = ['title','category','qty','price','comment','repeat','photo','photos','selectedPhotoIndex'];
    const update = {};
...
app.delete('/api/shopping/:id', auth, async (req, res) => {
  try {
    await db.collection('shopping').deleteOne({ _id: new ObjectId(req.params.id), ...shopScope(req.user) });
    await db.collection('listentries').deleteMany({ articleId: req.params.id, ...leScope(req.user) });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ── LIST ENTRIES ──────────────────────────────────
// {articleId, listId ('wocheneinkauf' | shoplist._id), checked}
  ...
  authFn(req, res, async () => {
    try {
      ...
      const r = await db.collection('listentries').findOneAndUpdate(
        { _id: new ObjectId(req.params.id), ...leScope(req.user) },
...
// Gast: alle entries + artikel für seine Liste
app.get('/api/guest/listentries', guestAuth, async (req, res) => {
  try {
    const entries = await db.collection('listentries').find({ ...leScope(req.user), listId: req.guestListId }).toArray();
    const articleIds = [...new Set(entries.map(e => e.articleId))];
    const articles = await db.collection('shopping').find({
      _id: { $in: articleIds.map(id => { try { return new ObjectId(id); } catch { return null; } }).filter(Boolean) }
    }).toArray();
    const artMap = new Map(articles.map(a => [a._id.toString(), { ...a, _id: a._id.toString() }]));
    res.json(entries.map(e => ({ ...e, _id: e._id.toString(), article: artMap.get(e.articleId) || null })));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
// Gast: Artikel anlegen → Kategorie = Listenname, entry für Gastliste anlegen
app.post('/api/guest/shopping', guestAuth, async (req, res) => {
  try {
    const { title, qty } = req.body;
    if (!title?.trim()) return res.status(400).json({ error: 'Titel fehlt' });
    const doc = {
      ...shopScope(req.user), title: title.trim(), category: req.guestListName,
      qty: qty || '', price: '', comment: '', photos: [], photo: null, repeat: null,
      selectedPhotoIndex: null, createdAt: new Date()
    };
    const r = await db.collection('shopping').insertOne(doc);
    const articleId = r.insertedId.toString();
    const leDoc = { ...leScope(req.user), articleId, listId: req.guestListId, checked: false, createdAt: new Date() };
...
app.get('/api/shoplists', auth, async (req, res) => {
  try {
    const lists = await db.collection('shoplists').find(shopScope(req.user)).sort({ createdAt: -1 }).toArray();
    res.json(lists.map(l => ({ ...l, _id: l._id.toString() })));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/shoplists', auth, async (req, res) => {
  try {
    // Max 3 Gastlisten (+ Wocheneinkauf = 4 gesamt)
    const existing = await db.collection('shoplists').countDocuments({ ...shopScope(req.user), expiresAt: { $gt: new Date() } });
    if (existing >= 3) return res.status(400).json({ error: 'Maximal 3 Gastlisten gleichzeitig' });
    const { name, hours } = req.body;
    if (!name?.trim()) return res.status(400).json({ error: 'Name fehlt' });
    const validHours = [24, 48, 72, 168, 720];
    const h = validHours.includes(Number(hours)) ? Number(hours) : 24;
    const token = genToken();
    const doc = {
      ...shopScope(req.user), name: name.trim(), token,
      createdBy: req.user._id.toString(), hours: h,
      expiresAt: new Date(Date.now() + h * 3600000), createdAt: new Date()
    };
    const r = await db.collection('shoplists').insertOne(doc);
    const listId = r.insertedId.toString();
    // Kopie aller aktuellen Wocheneinkauf-listentries für neue Liste anlegen
    const weekEntries = await db.collection('listentries').find({ ...leScope(req.user), listId: 'wocheneinkauf' }).toArray();
    if (weekEntries.length > 0) {
      await db.collection('listentries').insertMany(weekEntries.map(e => ({
        ...leScope(req.user), articleId: e.articleId, listId, checked: false, createdAt: new Date()
      })));
    }
    res.json({ ...doc, _id: listId });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/shoplists/:id', auth, async (req, res) => {
  try {
    ...
    await db.collection('listentries').deleteMany({ ...leScope(req.user), listId: list._id.toString() });
    await db.collection('checklogs').deleteMany({ ...leScope(req.user), listId: list._id.toString() });
...
app.post('/api/checklog', async (req, res) => {
  ...
  authFn(req, res, async () => {
    try {
      const { itemId, itemTitle, action, actor, listId } = req.body;
      await db.collection('checklogs').insertOne({
        ...leScope(req.user), itemId, itemTitle, action, actor,
        listId: listId || (req.isGuest ? req.guestListId : 'wocheneinkauf'),
        createdAt: new Date()
      });
      res.json({ ok: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
...
app.get('/api/checklog/:listId', async (req, res) => {
  const isGuest = !!req.headers['x-guest-token'];
  const authFn = isGuest ? guestAuth : auth;
  authFn(req, res, async () => {
    try {
      const logs = await db.collection('checklogs')
        .find({ ...leScope(req.user), listId: req.params.listId })
...
app.put('/api/mealplan', auth, async (req, res) => {
  try {
    ...
    const plan = {};
    ['mo','di','mi','do','fr','sa','so'].forEach(d => { plan[d] = (req.body[d] || '').trim(); });
    await db.collection('mealplans').updateOne(sc, { $set: { plan, updatedAt: new Date() } }, { upsert: true });
    res.json(plan);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/recipes', auth, async (req, res) => {
  try {
    const items = await db.collection('recipes').find(shopScope(req.user)).sort({ createdAt: -1 }).toArray();
    res.json(items.map(i => ({ ...i, _id: i._id.toString() })));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/recipes', auth, async (req, res) => {
  try {
    ...
    if (!name?.trim()) return res.status(400).json({ error: 'Name fehlt' });
    const doc = { ...shopScope(req.user), name: name.trim(), category: category || 'Sonstiges', link: link || '', ingredients: Array.isArray(ingredients) ? ingredients : [], notes: notes || '', photo: photo || null, createdAt: new Date() };
...
app.patch('/api/recipes/:id', auth, async (req, res) => {
  try {
    ...
    const update = {};
    allowed.forEach(f => { if (req.body[f] !== undefined) update[f] = req.body[f]; });
    const r = await db.collection('recipes').findOneAndUpdate({ _id: new ObjectId(req.params.id), ...shopScope(req.user) }, { $set: update }, { returnDocument: 'after' });
    if (!r) return res.status(404).json({ error: 'Nicht gefunden' });
    res.json({ ...r, _id: r._id.toString() });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/recipes/:id', auth, async (req, res) => {
  try {
    await db.collection('recipes').deleteOne({ _id: new ObjectId(req.params.id), ...shopScope(req.user) });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/poll', auth, async (req, res) => {
  try {
    const tasks = await db.collection('tasks').find(scope(req.user)).toArray();
    const users = await db.collection('users').find(req.user.groupId ? { groupId: req.user.groupId } : { _id: req.user._id }).project({ token: 0, notifications: 0 }).toArray();
    const stateKey = req.user.groupId || req.user._id.toString();
    const appstate = await db.collection('appstate').findOne({ _id: stateKey });
    const shopLast = await db.collection('shopping').find(shopScope(req.user)).sort({ createdAt: -1 }).limit(1).toArray();
    const leCount = await db.collection('listentries').countDocuments(leScope(req.user));
    const shopStamp = (shopLast[0]?.createdAt?.getTime() || 0) + leCount;
    const hash = crypto.createHash('md5').update(JSON.stringify(tasks) + JSON.stringify(users) + JSON.stringify(appstate || {}) + shopStamp).digest('hex');
...
app.get('/api/calendar/feed.ics', async (req, res) => {
  try {
    ...
    const sc = user.groupId ? { groupId: user.groupId } : { groupId: null, ownerId: uid };
    ...
    function getMondayOf(dateStr) { const d = new Date(dateStr + 'T00:00:00'); const day = d.getDay(); const diff = (day === 0 ? -6 : 1 - day); d.setDate(d.getDate() + diff); return d; }
    ...
    for (const t of allTasks) {
      ...
      if (isMine) { if (t.assignee !== uid || !t.deadline) continue; myEntries.push({ uid: tid, title: t.title, label: '✓', deadline: t.deadline, prio: t.prio || '–', cat: t.cat || '–' }); }
      else if (isShared) { const involved = (t.sharedWith || []).includes(uid) || t.creatorId === uid; if (!involved || !t.deadline) continue; myEntries.push({ uid: tid, title: t.title, label: '👥', deadline: t.deadline, prio: t.prio || '–', cat: t.cat || '–' }); }
      else if (isPool) {
        const assign = poolAssigns[tid];
        if (!assign || !t.deadline) continue;
        if (t.subtasks?.length > 0 && assign.subtaskAssignments) { const mySubs = t.subtasks.filter(s => !s.done && assign.subtaskAssignments[s.id] === uid); for (const sub of mySubs) myEntries.push({ uid: `${tid}-${sub.id}`, title: `${t.title}: ${sub.title}`, label: '🔄', deadline: t.deadline, prio: t.prio || '–', cat: t.cat || '–' }); }
        else { if (assign.assignedUser !== uid) continue; myEntries.push({ uid: tid, title: t.title, label: '🔄', deadline: t.deadline, prio: t.prio || '–', cat: t.cat || '–' }); }
      }
    }
    const todayD = new Date(); todayD.setHours(0,0,0,0);
    const diffToMon = todayD.getDay() === 0 ? -6 : 1 - todayD.getDay();
    const thisMonday = new Date(todayD); thisMonday.setDate(todayD.getDate() + diffToMon);
    const myPoolTitles = [];
    for (const t of allTasks) {
      ...
      const assign = poolAssigns[tid];
      if (!assign) continue;
      if (t.subtasks?.length > 0 && assign.subtaskAssignments) { t.subtasks.filter(s => !s.done && assign.subtaskAssignments[s.id] === uid).forEach(s => myPoolTitles.push(`${t.title}: ${s.title}`)); }
      else { if (assign.assignedUser === uid && !t.done) myPoolTitles.push(t.title); }
    }
    const lines = ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//TaskFlow//DE','CALSCALE:GREGORIAN',`X-WR-CALNAME:TaskFlow – ${user.name || 'Meine Aufgaben'}`,'X-WR-CALDESC:Persönliche Aufgaben aus TaskFlow','REFRESH-INTERVAL;VALUE=DURATION:PT1H','X-PUBLISHED-TTL:PT1H'];
    const byWeek = {};
    for (const e of myEntries) { const mon = getMondayOf(e.deadline); const key = toDateStr(mon); if (!byWeek[key]) byWeek[key] = { mon, entries: [] }; byWeek[key].entries.push(e); }
    for (const [weekKey, { mon, entries }] of Object.entries(byWeek)) {
      ...
      lines.push('BEGIN:VEVENT', `UID:week-${weekKey}-${uid}@taskflow`, `SUMMARY:${icalEscape('📋 Meine Aufgaben (' + entries.length + ')')}`, `DTSTART;VALUE=DATE:${toDateStr(mon)}`, `DTEND;VALUE=DATE:${toDateStr(sun)}`, `DESCRIPTION:${icalEscape(desc.trim())}`, 'TRANSP:TRANSPARENT', 'STATUS:CONFIRMED', 'END:VEVENT');
    }
    if (myPoolTitles.length > 0) {
      const poolDesc = myPoolTitles.map(t => `• ${t}`).join('\n');
      for (let i = 0; i < 7; i++) { const day = new Date(thisMonday); day.setDate(thisMonday.getDate() + i); const nextDay = new Date(day); nextDay.setDate(day.getDate() + 1); lines.push('BEGIN:VEVENT', `UID:pool-week-${toDateStr(thisMonday)}-day${i}-${uid}@taskflow`, `SUMMARY:${icalEscape('TaskFlow - Poolaufgaben')}`, `DTSTART;VALUE=DATE:${toDateStr(day)}`, `DTEND;VALUE=DATE:${toDateStr(nextDay)}`, `DESCRIPTION:${icalEscape(poolDesc)}`, 'TRANSP:TRANSPARENT', 'STATUS:CONFIRMED', 'END:VEVENT'); }
    }
    for (const e of myEntries) { const nextDay = new Date(e.deadline + 'T00:00:00'); nextDay.setDate(nextDay.getDate() + 1); lines.push('BEGIN:VEVENT', `UID:dl-${e.uid}@taskflow`, `SUMMARY:${icalEscape(e.label + ' ' + e.title)}`, `DTSTART;VALUE=DATE:${e.deadline.replace(/-/g,'')}`, `DTEND;VALUE=DATE:${toDateStr(nextDay)}`, `DESCRIPTION:${icalEscape('Kategorie: ' + e.cat + '\nPriorität: ' + e.prio)}`, 'TRANSP:TRANSPARENT', 'STATUS:CONFIRMED', 'END:VEVENT'); }
